...
module.exports = function() {
                        ...
                        if(!_.has(record,"deleted")) {
                            if(!_.isEqual(record,result)) {
                                checkConflict(record,function(st) {
                                    if(!st) {
                                        /*
                                        console.warn("WARN : FLAGGED UPDATE ANOMALY ON CHECK, Logging into database...");
                                        queue.addConflict(result,record,function(st){
    ...
    //resolve conflicts
    //action 1 : revert to prev update
    //action 2 : put your update
    //action 3 : manual merge

    function resolveConflict(id,action,msg,callback) {
        queue.getConflicts(function(st,data) {
            ...
            if(!_.isEmpty(conflict)&& _.isObject(conflict)) {
                if(action == 1) {
                    ...
                    //manual merge...
                    var merge = _.assign(msg, _.pick(conflict.data,["_id","timeStamp"]));
                    queue.addQueue(conflict.document,merge,"force_update",function(st) {
                        if(st) {
                            console.log("Data Updated by merged copy as per user choice!");
                            queue.resolveConflict(conflict.timeStamp,function(st) {
                                if(st) {
                                    console.log("Conflict Resolved from Queue at " + new Date().getTime());
                                    callback(true);
