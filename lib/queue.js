                        ...
                        remoteDB.readOne(data.document,data.data._id,function(rec,st) {
                            if(st) {
                                ...
                                if(data.timeStamp < rec.timeStamp) {
                                    //timeStamp of the updated doc is less than the timestamp of the doc in server,
                                    // which implies update anomaly
                                    console.warn("WARN : FLAGGED UPDATE ANOMALY, RESOLVING...");
                                    var updatedDoc = data.data;
                                    defaults(updatedDoc,rec);
                                    console.log("Updated doc " + JSON.stringify(updatedDoc));
                                    remoteDB.update(data.document,updatedDoc,function(status) {
                                        if(status) {
                                            console.log("Data updated by new doc at " + new Date().getTime());
                                            //delete from queue
                                            localQueue.indexedDB.deleteQueue(data.timeStamp,function(status) {
                                                console.log(status);
                                                callback(true);
