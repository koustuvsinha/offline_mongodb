    ...
    localDB.getAllConflicts(function(st,data) {
        if(st) {
            ...
            data.forEach(function(conflict) {
                ...
                html += "<div class='row'><div class='col-md-5'><button class='btn btn-primary' onclick=resolveData(" + conflict.timeStamp +",false)>Keep your update</button>" +
                    "&nbsp;<button class='btn btn-danger' onclick=resolveData(" + conflict.timeStamp +",true)>Revert to Server Value</button></div></div>";
                html += "<hr/>";
            })
            document.getElementById("table").innerHTML = html;
        }
...
function resolveData(id,action) {
    localDB.resolveConflict(id,action,function(st) {
        if(st) {
